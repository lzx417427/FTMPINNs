...
class Net(nn.Module):
    ...
    def forward(self,x):#有两个特征的样本 x1的形状为（batch_size,2） [2000,2]
        ...
        # out_final= 2 * (out_final[:, 0])**2 + 2 *(out_final[:, 1])**2 + 2 * out_final[:, 0] + 2 * out_final[:, 1]
        return out_final
        # ut=d_t = u_tx[:, 0].unsqueeze(-1)
        # ut_alpha = torch.pow(ut, self.alpha)
...
pt_two_rig=Variable(torch.from_numpy(two_rig).float(), requires_grad=True)




t_in_var = np.random.uniform(low=0, high=1.0, size=(100, 1))
x_bc_var = np.random.uniform(low=0, high=1.0, size=(100, 1))
u_bc_sin = x_bc_var ** 2
...
pt_x_in_pos_zero = Variable(torch.from_numpy(x_in_pos_zero).float(), requires_grad=False)
...
# 将误差损失加起来
    ...
    # if epoch == iterations-1:
         # torch.save(net.state_dict(),'./model_weight/example1_no_noise_0.5prediction.pth')
...
#加载网络：
model=Net(256)
# model.load_state_dict(torch.load('./model_weight/example1_no_noise_0.5prediction.pth'))
