...
def interior(n=N):
    ...
    two=2*one
    cond = ((2/math.gamma(2.8))*y**1.8-(2*y**2)-two) * torch.exp(x+z)
...
def fenshu(u):
    ...
    # return loss((1/gamma(0.5))*d_result -gradients(uxy, x, 2)-gradients(uxy, z, 2),cond)
...
# print('min:', min, '  epo:', epochs)
    ...
    if l < best_loss:
         #torch.save(u.state_dict(), 'np_weight/daxiu_best_model1.pth')
         # torch.save(u.state_dict(), 'np_weight/example3_no_noise_0.5.pth')#还没跑
         # torch.save(u.state_dict(), 'np_weight/example3_no_noise_0.2.pth')
         best_loss = l
    if i % 50 == 0:
        print(i, "l", l)
    # if i % 100 == 0:
    #     print(i,"l",l)
# np.save('./np_weight/0.5_daxiu_2Dl2error.npy', l2_loss_list)
# np.save('./np_weight/daxiu_2Dl2error.npy', l2_loss_list)#0.2
...
plt.title(r'$\alpha=0.2$, Prediction: $u(x,y,t)$')
#tt=torch.zeros_like(yy)
plt.show()
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
...
ax.set_zlabel('t')
fig.colorbar(img, ax=ax)
plt.title(r'$\alpha=0.2$, abs error')
